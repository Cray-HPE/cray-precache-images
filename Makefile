...
all: lint dep-up test package image
...
lint:
	CMD="lint charts/cray-precache-images" $(MAKE) helm

dep-up:
	CMD="dep up charts/cray-precache-images" $(MAKE) helm
...
ifdef CHART_VERSIONS
	CMD="package charts/cray-precache-images --version $(word 1, $(CHART_VERSIONS)) -d packages" $(MAKE) helm
else
...
images:
	CHART=charts/cray-precache-images $(MAKE) -s extracted-images annotated-images | sort -u
